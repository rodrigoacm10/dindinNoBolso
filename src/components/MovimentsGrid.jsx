...
function MovimentsGrid({
  ...
  const columns = [
    ...
    //   getOptionLabel: (value) => value.nome,
    //   valueOptions: ({ row }) =>
    //     row.typeMov == "receita" ? [...revCats] : [...expCats],
    //   width: 130,
    //   // vai ser mais ou menos assim
    //   // valueOptions: ({ row }) => {
    //   //   if (row.valor > 20) {
    ...
    //   field: "country",
    //   // type: "singleSelect",
    //   // getOptionValue: (value) => value.categoria_receita_id,
    //   // getOptionLabel: (value) => value.nome,
    //   // valueOptions: [...revCats],
    //   renderCell: ({ row }) => {
    //     return row.valor;
    //   },
    //   editable: true,
  ...
  const getMovs = async () => {
    ...
    // novo
    // console.log([catsRevArr]);
    // setRevCats([...catsRevArr]);
    // setExpCats([...catsExpArr]);
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        alignItems="center"
      >
        {report && <p className="text-white">Movimentações no periodo</p>}
        <Box
          // width="100%"
          display="flex"
